...
def load_data():
    ...
    3. Mock data (if nothing found, for demonstration)
    """
    test_cases = []
        ...
        try:
            with open("llm.txt", "r") as f:
                ...
                for test_id, attrs in data.items():
                    test_cases.append({
                        'id': test_id,
                        'magnitude': float(attrs.get('relevance', 0.5)),
                        'frequency': float(attrs.get('complexity', 0.5)),
                        'original_semantics': "From LLM" 
                    })
...
def interference_aware_fitness(permutation, test_cases_lookup):
    ...
    for t in range(1, time_steps + 1):
        ...
        for position_index, test_id in enumerate(permutation):
            ...
            A = test_case['magnitude']  # Relevance
...
def generate_report(best_sequence, tc_map, n):
    ...
    report += "| Priority | Test Case ID | Magnitude | Position Phase (rad) | Semantics |\n"
    report += "|---|---|---|---|---|\n"
    ...
    for rank, tid in enumerate(best_sequence):
        ...
        report += f"| {rank+1} | {t['id']} | {t['magnitude']:.2f} | {phase:.3f} | {t.get('original_semantics', '')} |\n"
